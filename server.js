...
const { bot, deliverOrder, notifyAdmins } = require('./bot'); // starts the bot (polling) as a side effect
...
app.get('/api/dashboard', requireAdmin, (req, res) => {
  const orders = dbLayer.db.get('orders').value();
  const today = new Date().setHours(0, 0, 0, 0);
  const todayOrders = orders.filter(o => o.createdAt >= today);
  res.json({
    totalUsers: dbLayer.db.get('users').size().value(),
    activeUsers: dbLayer.db.get('users').filter(u => Date.now() - u.lastActivity < 7 * 24 * 3600 * 1000).size().value(),
    ...
    totalStock: dbLayer.db.get('vouchers').filter({ status: 'AVAILABLE' }).size().value(),
    soldVouchers: dbLayer.db.get('vouchers').filter({ status: 'SOLD' }).size().value(),
    lowStockProducts: dbLayer.db
      .get('products')
      .filter(p => dbLayer.availableStockCount(p.id) <= (p.lowStockThreshold || 5))
      .size()
      .value(),
  });
});
...
app.post('/api/broadcast', requireAdmin, async (req, res) => {
  ...
  dbLayer.db.get('broadcasts').push({ id: dbLayer.id('bcast'), text, sent, failed, total: users.length, createdAt: Date.now() }).write();
...
app.get('/pay/:orderId', async (req, res) => {
  ...
  try {
    ...
    const checksum = await PaytmChecksum.generateSignature(JSON.stringify(paytmParams.body), config.paytm.merchantKey);
    paytmParams.head = { signature: checksum };

    const response = await axios.post(`${config.paytm.initiateTxnUrl}${order.id}`, paytmParams, {
      headers: { 'Content-Type': 'application/json' },
    });

    const txnToken = response.data?.body?.txnToken;
    if (!txnToken) {
      console.error('Paytm initiate failed:', response.data);
...
app.post('/paytm/callback', async (req, res) => {
  ...
  try {
    ...
    const statusBody = { mid: config.paytm.mid, orderId };
    const statusChecksum = await PaytmChecksum.generateSignature(JSON.stringify(statusBody), config.paytm.merchantKey);
    const statusResp = await axios.post(
      config.paytm.transactionStatusUrl,
      { body: statusBody, head: { signature: statusChecksum } },
      { headers: { 'Content-Type': 'application/json' } }
    );
    const result = statusResp.data?.body;
